...
// This class manages the structure for vertex data
...
namespace Dvtx
{
	...
	public:
		...
		template<ElementType> struct Map; //a struct templated on ElementType 
		template<> struct Map<Position2D> //this feels like a case look up table
		{
	...
	private:
		std::vector<Element> elements;// this is the actual layout of our structure
	};

	class Vertex  //this is a proxy view into the layout data
	{
	...
	public:
		...
		template<typename T>
		void SetAttributeByIndex(size_t i, T&& val) noxnd//universal reference, perfect forwarding
		{
	...
	private:
		...
		void SetAttributeByIndex(size_t i, First&& first, Rest&&... rest) noxnd//template recursion, split the args into First and rest
		{
	...
	private:
		char* pData = nullptr; //this point to the beginning of that vertex data
		const VertexLayout& layout; //reference to the layout structure
	};
	...
	public:
		...
		//T&&... means args, it can be one arg or multiple args. Since you can't looping through all these args, you have to use recursion!!!! 
		{
	...
	private:
		std::vector<char> buffer;//actual buffer of bytes
		VertexLayout layout;//Structureof how the buffer is constructed
	};
