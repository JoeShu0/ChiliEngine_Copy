...
namespace Bind
{
		...
		count((UINT)indices.size())
	{
		...
		ibd.BindFlags = D3D11_BIND_INDEX_BUFFER;//bind to indexbuffer
		...
		isd.pSysMem = indices.data();//data()return ptr to the data
		GFX_THROW_INFO(GetDevice(gfx)->CreateBuffer(&ibd, &isd, pIndexBuffer.GetAddressOf()));
	}
